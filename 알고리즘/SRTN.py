        ...
        if self.isEmpty(): return None
            ...
            for i in range(1, self.size()): # 모든 항목에 대해서
                if self.items[i].rt < self.items[highest].rt:   # rest Time이 가장 낮으면 가장 높은 우선순위
                    highest = i         # 인덱스 갱신
                elif self.items[i].rt == self.items[highest].rt: # 만약 resetTime이 같으면 먼저 온게 가장 높은 우선순위 
...
class process:
    ...
    def __str__(self):
        return " #Process" + str(self.id) + " ID = " + str(self.id) + " AT = " + str(self.at) + " BT = " + str(self.bt) + " RT = " + str(self.rt)  + " TT = " + str(self.tt) + " NTT = " + str(self.ntt) + " WT = " + str(self.wt)
    ...
    for i in range(len(processor_ls)):                                           # 프로세서 수 만큼 순위 매기기
        ...
        for j in range(len(processor_ls)):
            if to_go_process == processor_ls[j].process:                                       # 우선순위(to_go_process)가 cpu안에 있으면
                ...
                break 
            elif to_go_process != processor_ls[j].process and processor_ls[j].lock==False:     # 우선순위가 cpu안에 없고, 안잠겨있으면
